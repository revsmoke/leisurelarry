...
func _run() -> void:
	...
	_check(not app.game.completed and bridge.observation().score < 100, "Friendship remains a valid choice without completing or awarding the finale")
	await _act("Talk Eve · rooftop host")
	await _choice("ending_flirt")
