...
func _run() -> void:
	...
	var final_autosave := FileAccess.get_file_as_bytes(AUTOSAVE)
	var again := _find_button(app.modal, "One more evening")
	_check(again != null, "Ending exposes new-evening button")
	if again != null:
		again.pressed.emit()
	_check(app.setup_open and app.game.completed and app.game.score == 100, "One-more-evening button opens a profile draft before replacing progress")
	_check(FileAccess.get_file_as_bytes(AUTOSAVE) == final_autosave, "Opening a profile draft preserves the completed autosave")
	var keep := _find_button(app.modal, "Keep this evening")
	_check(keep != null, "Profile draft offers cancellation")
	...
	var fresh: Variant = JSON.parse_string(FileAccess.get_file_as_string(AUTOSAVE))
	_check(fresh is Dictionary and fresh.profile == {"character": "larry", "orientation": "bisexual"} and fresh.room == "street" and fresh.score == 0 and not fresh.completed, "New-night commitment autosaves the fresh identity and progress")
	app.queue_free()
	await process_frame
